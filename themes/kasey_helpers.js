...
// Text on a coloured rounded shape (for reveal answers, labels, etc.)
function addTextOnShape(slide, text, shapeOpts, textOpts) {
  const { x, y, w, h } = shapeOpts;
  slide.addShape("roundRect", {
    x, y, w, h,
    rectRadius: shapeOpts.rectRadius || 0.08,
    fill: shapeOpts.fill || { color: C.TEAL },
  });
  const to = textOpts || {};
  slide.addText(text, {
    x, y, w, h,
...
/**
 * titleSlide — Dark full-bleed title for session start.
 */
function titleSlide(pres, title, subtitle, meta, notes) {
  const s = pres.addSlide();
  s.background = { color: C.BUSH };
  // Vertical ochre accent bar
  s.addShape("rect", { x: 0, y: 0, w: 0.12, h: SLIDE_H, fill: { color: C.OCHRE } });
  // Decorative shapes
...
function liSlide(pres, liItems, scItems, notes, footer) {
  ...
  addTitle(s, "Learning Intention & Success Criteria");
  ...
  addCard(s, 0.5, CONTENT_TOP, 9, liH, { strip: C.BUSH });
  s.addText("Learning Intention", {
    x: 0.75, y: CONTENT_TOP + 0.07, w: 5, h: 0.30,
    fontSize: 13, fontFace: FONT_B, color: C.BUSH, bold: true, margin: 0,
  });
  ...
  addCard(s, 0.5, scY, 9, scH, { strip: C.OCHRE });
  s.addText("Success Criteria \u2014 I can\u2026", {
    x: 0.75, y: scY + 0.07, w: 5, h: 0.28,
    fontSize: 13, fontFace: FONT_B, color: C.OCHRE, bold: true, margin: 0,
  });
...
function contentSlide(pres, badgeText, badgeColor, title, bullets, notes, footer, drawRight) {
  const s = pres.addSlide();
  addTopBar(s, C.BUSH);
  addBadge(s, badgeText || "Content", { color: badgeColor || C.BUSH });
  addTitle(s, title);

  const cardW  = drawRight ? 5.4 : 9.0;
  const cardH  = SAFE_BOTTOM - CONTENT_TOP;
  addCard(s, 0.5, CONTENT_TOP, cardW, cardH, { strip: badgeColor || C.BUSH, fill: C.WHITE });
...
/**
 * discussionSlide — Discussion prompt slide with question card(s).
 */
function discussionSlide(pres, title, prompts, notes, footer) {
  const s = pres.addSlide();
  addTopBar(s, C.TEAL);
  addBadge(s, "Discussion", { color: C.TEAL });
  addTitle(s, title || "Discuss with Your Partner");

  const availH  = SAFE_BOTTOM - CONTENT_TOP;
  const qH      = Math.min(0.95, (availH - 0.1) / Math.max(prompts.length, 1));
  const fontSize = prompts.length > 4 ? 13 : 15;

  const stripColors = [C.BUSH, C.TEAL, C.OCHRE, C.CORAL];
  prompts.forEach((q, i) => {
    const y = CONTENT_TOP + i * (qH + 0.10);
    if (y + qH > SAFE_BOTTOM) return;
    addCard(s, 0.5, y, 9, qH, { strip: stripColors[i % stripColors.length], fill: C.WHITE });
    s.addText(q, {
...
/**
 * taskSlide — "First, Next, Then" guided/independent practice task.
 */
function taskSlide(pres, badgeText, title, steps, notes, footer) {
  const s = pres.addSlide();
  addTopBar(s, C.SAGE);
  addBadge(s, badgeText || "Your Turn", { color: C.SAGE });
  ...
  const labelColors = [C.BUSH, C.TEAL, C.SAGE, C.OCHRE];
...
/**
 * closingSlide — Dark reflection/review slide for session close.
 */
function closingSlide(pres, reflectionPrompt, takeaways, notes) {
  ...
  s.addText("Turn & Talk", {
    ...
    fontSize: 15, fontFace: FONT_B, color: C.OCHRE, bold: true, margin: 0,
  });
  ...
  if (takeaways && takeaways.length) {
    s.addText("Key Takeaways", {
      ...
      fontSize: 14, fontFace: FONT_B, color: C.OCHRE, bold: true, margin: 0,
    });
