// Matilda Helpers — Adapter module
// Wraps the factory theme system for the Matilda Year 3/4 Literacy unit (Week 5).
...
// Week 5, Grade 3/4, Literacy → variant 4 ("Autumn Library")
const T = createTheme("literacy", "grade34", weekToVariant(5));
const { C, FONT_H, FONT_B } = T;

// Legacy colour aliases used by existing Matilda build scripts.
// These map descriptive names to the semantic palette keys.
C.PLUM      = C.PLUM      || C.PRIMARY;    // deep dominant colour
C.HONEY     = C.HONEY     || C.ACCENT;     // warm accent highlight
C.PARCHMENT = C.PARCHMENT || C.BG_LIGHT;   // light warm background
C.CORAL     = C.CORAL     || C.ALERT;      // emphasis / attention
C.SAGE      = C.SAGE      || C.SUCCESS;    // green accent
C.AMBER     = C.AMBER     || C.ASSESS;     // third accent for multi-item lists
C.LIGHT     = C.LIGHT     || C.BG_LIGHT;   // light background alias
